...
* Добавлен просмотр списка DNS зон и записей в ней (через icm, модуль не требуется) на хосте с ролью DNS-сервер, для быстрого поиска. Добавлен лог питания (перезагрузки и выключений).
...
> MAC-адрес выводится и берется для Magic Packet из формы отправки сообщения пользователям.
