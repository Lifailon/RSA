...
### **Поиск MAC-адреса компьютера** которые уже не доступен.
Производится по средствам просмотра ARP-таблиц на других сервера в кач-ве proxy, или вывод таблицы сервера с ролью DHCP (установка модуля не требуется) для поиска MAC-адреса. Используется в связке с последующей отправкой Magic Packet.
